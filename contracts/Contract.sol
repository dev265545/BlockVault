...
contract Contract{
    ...
    function addCase(string calldata judge_id,string calldata law_id,string calldata cli_id, string calldata cnr, string calldata _docLink, string calldata date) external{
    ...
    function updateCase(string calldata user_id, string calldata cnr, string calldata _docLink, string calldata date) external{
        ...
        for(uint i = 0; i < lawyer[user_id].length; i++){
            if(keccak256(abi.encodePacked(lawyer[user_id][i].cnr)) == keccak256(abi.encodePacked(cnr))){
                lawyer[user_id][i].docLink = _docLink;
                lawyer[user_id][i].date = date;
            }
        }
        for(uint i = 0; i < client[user_id].length; i++){
            if(keccak256(abi.encodePacked(client[user_id][i].cnr)) == keccak256(abi.encodePacked(cnr))){
